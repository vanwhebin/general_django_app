...
from libs.qywx.conf import DEBUG
...
class AbstractApi(object):
	...
	def __httpPost(self, url, args):
		...
		return requests.post(realUrl, data=json.dumps(args, ensure_ascii=False).encode('utf-8'), verify=False).json()
	...
	def __httpGet(self, url):
		...
		return requests.get(realUrl, verify=False).json()
